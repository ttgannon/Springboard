...
function extractValue(arr, key) {
    return arr.reduce(function(people, current) {
        people.push(current[key]);
        return people;
    }, [])
}
...
and returns an object with the keys as the vowel and the values as the number of 
times the vowel appears in the string. This function should be 
...
function vowelCount(str) {
    let vowels = 'aeiou';
    return str.split("").reduce(function(object,letter) {
        letter.toLowerCase();
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function(object, currentItem, index) {
        console.log(index);
...
the value should be placed in the first subarray. If the result of the 
callback function at that specific value is false, 
